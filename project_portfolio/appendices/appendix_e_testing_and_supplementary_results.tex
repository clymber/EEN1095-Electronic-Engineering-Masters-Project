...
For a decibel (dB) response with $N$ evaluated values, the reported mean
...
\end{equation}
This $S_{7,1}$-only quantity must not be confused with the native six-path
deep-null values stored by the Curve and Full-S-Matrix runs.

For complete complex matrices, with each $S$-parameter counted once per design,
frequency and port pair, the complex-domain MAE and normalised RMSE (NRMSE) are
\begin{align}
...
record. The multilayer perceptron abbreviation (MLP) is used in model labels.
All values were recomputed by Notebook 07 (NB07), the selected-model evaluation
notebook, from the runs named in Table~\ref{tab:appendix-selected-runs}.
...
\begin{table*}[t]
    ...
    \begin{tabular}{@{}llrrrr@{}}
        ...
        \midrule
        validation & simulation & $3.41\times10^{-11}$ & 0/281,200
                   & 0.999998 & 0.318476 \\
        validation & prediction & 0 & 0/281,200
                   & 0.924104 & 0.481261 \\
        test & simulation & $7.00\times10^{-11}$ & 0/281,200
             & 0.999978 & 0.317284 \\
...
The evaluation inputs needed for reproduction are the named dataset, project
seed 128, the 4,218/1,406/1,406 design split, the eight selected run manifests
and the paired NB07 source/notebook. Earlier NB03--NB06 development inspected
test results, so this record supports retrospective held-out evaluation rather
than an untouched final confirmation.
